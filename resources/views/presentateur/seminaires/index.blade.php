...
<head>
    ...
    <title>Mes Séminaires Programmés et Passés - {{ config('app.name', 'Laravel') }}</title>
...
<body class="font-sans antialiased min-h-screen flex flex-col">
    <div class="w-full flex flex-col items-center min-h-screen">
        ...
        <div class="hero-main animate-fade-in">
            <h1 class="text-2xl font-bold text-indigo-800 mb-8 text-center">Mes Séminaires Programmés et Passés</h1>
            @if (session('success'))
                            ...
                            @foreach($seminaires as $seminaire)
                                <tr class="transition-all duration-200 hover:bg-indigo-50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $seminaire->date_presentation ? $seminaire->date_presentation->format('d/m/Y') : 'N/D' }}
                                        @if($seminaire->heure_debut) à {{ \Carbon\Carbon::parse($seminaire->heure_debut)->format('H:i') }}@endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        @if($seminaire->resume)
                                            <span class="text-green-600">Soumis</span>
                                        @elseif($seminaire->statut == 'programmé')
                                            <span class="text-red-600">À soumettre</span>
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                         <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full capitalize
                                            @if($seminaire->statut == 'passé') bg-gray-100 text-gray-800
                                            @elseif($seminaire->statut == 'annulé') bg-red-100 text-red-800
                                            @else bg-blue-100 text-blue-800 @endif">
                                            {{ $seminaire->statut }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('seminaires.authentifie.show', $seminaire) }}" class="text-indigo-600 hover:text-indigo-900 transition-colors duration-200">Voir</a>
                                        @if($seminaire->statut == 'programmé')
                                            <a href="{{ route('presentateur.seminaires.resume.form', $seminaire) }}" class="ml-2 text-yellow-600 hover:text-yellow-900 transition-colors duration-200">
                                                {{ $seminaire->resume ? 'Modifier Résumé' : 'Soumettre Résumé' }}
                                            </a>
                                        @endif
                                    </td>
                                </tr>
            ...
            @else
                <p class="text-gray-500">Vous n'avez aucun séminaire programmé ou passé pour le moment.</p>
            @endif
        </div>
