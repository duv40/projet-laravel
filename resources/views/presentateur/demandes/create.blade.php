...
<head>
    ...
    <title>{{ isset($demandePresentation) ? 'Modifier la Demande de Présentation' : 'Nouvelle Demande de Présentation' }} - {{ config('app.name', 'Laravel') }}</title>
    ...
    <button type="submit" class="welcome-link" style="background:none;border:none;padding:0;cursor:pointer;">
        ...
        <div class="hero-main animate-fade-in">
            <h1 class="text-2xl font-bold text-indigo-800 mb-6">
                {{ isset($demandePresentation) ? 'Modifier la Demande de Présentation' : 'Nouvelle Demande de Présentation' }}
            </h1>
            <form method="POST" action="{{ isset($demandePresentation) ? route('presentateur.demandes.update', $demandePresentation) : route('presentateur.demandes.store') }}" enctype="multipart/form-data">
                @csrf
                @if(isset($demandePresentation))
                    @method('PUT')
                @endif
                ...
                <div class="mt-4">
                    ...
                    @error('document_joint')
                        <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
                    @enderror
                    @if(isset($demandePresentation) && $demandePresentation->document_joint)
                ...
                <div class="flex items-center justify-end mt-6">
                    ...
                    <button type="submit" class="px-6 py-2 bg-indigo-600 text-white font-semibold rounded-full shadow hover:bg-indigo-700 transition-all duration-200">
                        {{ isset($demandePresentation) ? 'Mettre à jour la demande' : 'Soumettre la demande' }}
                    </button>
                </div>
            </form>
        </div>
