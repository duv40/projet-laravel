...
<head>
    ...
    <title>Modifier un Utilisateur - {{ config('app.name', 'Laravel') }}</title>
...
<body class="font-sans antialiased min-h-screen flex flex-col">
    <div class="w-full flex flex-col items-center min-h-screen">
        ...
        <div class="hero-main animate-fade-in">
            <div class="hero-title">Modifier un Utilisateur</div>
            <div class="hero-desc">Modifiez les informations de l'utilisateur sélectionné.</div>
            <div class="form-bloc">
                ...
                <form method="POST" action="{{ route('admin.utilisateurs.update', $utilisateur) }}">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $utilisateur->name ?? '')" required autofocus autocomplete="name" />
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email', $utilisateur->email ?? '')" required autocomplete="username" />
                    ...
                    <div class="mt-4">
                        ...
                        <select name="role" id="role" required class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            @foreach($roles as $roleValue)
                                <option value="{{ $roleValue }}" {{ old('role', $utilisateur->role ?? '') == $roleValue ? 'selected' : '' }}>
                                    {{ ucfirst($roleValue) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="block mt-4">
                        <label for="recoit_notifications_seminaires" class="inline-flex items-center">
                            <input id="recoit_notifications_seminaires" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                                   ...
                                   {{ old('recoit_notifications_seminaires', $utilisateur->recoit_notifications_seminaires ?? false) ? 'checked' : '' }}>
